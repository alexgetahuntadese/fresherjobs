...
export default async function AdminDashboardPage({ searchParams }: DashboardPageProps) {
  ...
  const { data: applications } = await supabase.from('applications').select('id, job_id, full_name, email, phone, cv_url, cv_path, cover_letter, cover_letter_path, created_at').order('created_at',{ascending:false});
  const applicationRows=await Promise.all((applications??[]).map(async(a)=>{let cvLink=a.cv_url;let coverLetterLink=null;if(!cvLink&&a.cv_path){const {data}=await supabase.storage.from('cv-uploads').createSignedUrl(a.cv_path,3600);cvLink=data?.signedUrl??null;}if(a.cover_letter_path){const {data}=await supabase.storage.from('cv-uploads').createSignedUrl(a.cover_letter_path,3600);coverLetterLink=data?.signedUrl??null;}return {...a,cvLink,coverLetterLink};}));
